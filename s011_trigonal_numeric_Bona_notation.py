...
# for angle_applied in np.linspace(0,90, 90):

# Apply rotation
rotation = mechinterfabric.utils.get_rotation_by_vector(
    vector=5 * np.array([0, 0, 1]), degrees=True
)
...
for index in indices_eigenvectors_double_eigenvalues:
    ...
    print(f"Coincidence = {np.allclose(deviator_reconstructed_rotated, deviator_bona)}")
